/datum/biome/eden_mudlands
	turf_type = /turf/open/misc/dirt/jungle/dark
	flora_types = list(/obj/structure/flora/grass/jungle,/obj/structure/flora/grass/jungle/b, /obj/structure/flora/rock/jungle, /obj/structure/flora/rock/pile/largejungle)
	flora_density = 3

/datum/biome/eden_plains
	turf_type = /turf/open/misc/grass/jungle
	flora_types = list(/obj/structure/flora/grass/jungle,/obj/structure/flora/grass/jungle/b, /obj/structure/flora/tree/jungle, /obj/structure/flora/rock/jungle, /obj/structure/flora/junglebush, /obj/structure/flora/junglebush/b, /obj/structure/flora/junglebush/c, /obj/structure/flora/junglebush/large, /obj/structure/flora/rock/pile/largejungle, /obj/structure/flora/ausbushes)
	flora_density = 15

/datum/biome/eden_jungle
	turf_type = /turf/open/misc/grass/jungle
	flora_types = list(/obj/structure/flora/grass/jungle,/obj/structure/flora/grass/jungle/b, /obj/structure/flora/tree/jungle, /obj/structure/flora/rock/jungle, /obj/structure/flora/junglebush, /obj/structure/flora/junglebush/b, /obj/structure/flora/junglebush/c, /obj/structure/flora/junglebush/large, /obj/structure/flora/rock/pile/largejungle)
	flora_density = 40

/datum/biome/eden_jungle/deep
	flora_density = 65

/datum/biome/eden_wasteland
	turf_type = /turf/open/misc/dirt/jungle/wasteland
	flora_types = list(/obj/structure/flora/tree/dead, /obj/effect/decal/remains/human, /obj/effect/decal/remains/xeno, /obj/effect/decal/cleanable/robot_debris/old, /obj/effect/decal/remains/robot)
	flora_density = 3

/datum/biome/eden_water
	turf_type = /turf/open/water/jungle
	flora_types = list(/obj/structure/flora/ausbushes/reedbush)
	flora_density = 15

/datum/biome/eden_mountain
	turf_type = /turf/open/misc/ashplanet/rocky/planet
	flora_types = list(/obj/structure/flora/rock, /obj/structure/flora/rock/pile)
	flora_density = 15
